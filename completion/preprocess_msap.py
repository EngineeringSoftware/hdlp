...
# S2S-LHSPA1-{2,3,4,5}{+Type} models.
...
def build_save_dataset_msap(corpus_type, fields, src_reader, tgt_reader, opt):
    assert corpus_type in ['train', 'valid']

    abs_path = os.path.dirname(opt.train_src[0])
    from onmt.inputters.MultiSourceAPInputter import MultiSourceAPInputter
    from onmt.inputters.MultiSourceAPDataset import MultiSourceAPDataset
    ...
    if (corpus_type == "train" or opt.filter_valid):
        filter_pred = partial(MultiSourceAPInputter.filter_example,
    ...
    if corpus_type == "train":
        ...
        if opt.src_vocab != "":
            try:
                ...
                src_vocab, src_vocab_size = MultiSourceAPInputter.load_vocab(opt.src_vocab, "src", counters, opt.src_words_min_frequency)
            # end try
        else:
            src_vocab = None
        # end if
        ...
        if opt.tgt_vocab != "":
            tgt_vocab, tgt_vocab_size = MultiSourceAPInputter.load_vocab(opt.tgt_vocab, "tgt", counters, opt.tgt_words_min_frequency)
        else:
            tgt_vocab = None
        # end if
    # end if
    ...
    if corpus_type == "train":
        ...
        if existing_fields is None:
            fields = MultiSourceAPInputter.build_fields_vocab(
                opt.src_types,
                fields, counters,
...
def main_msap(opt):
    ...
    fields = MultiSourceAPInputter.get_fields(
        opt.src_types,
        src_nfeats,
        tgt_nfeats,
    ...
    build_save_dataset_msap(
        'train', fields, src_reader, tgt_reader, opt)

    if opt.valid_src and opt.valid_tgt:
        logger.info("Building & saving validation data...")
        build_save_dataset_msap('valid', fields, src_reader, tgt_reader, opt)
