...
from onmt.modules.MultiSourceAPGlobalAttention import MultiSourceAPGlobalAttention
from onmt.decoders.decoder import DecoderBase
from onmt.models.stacked_rnn import StackedLSTM, StackedGRU
from onmt.modules import context_gate_factory, GlobalAttention # GlobalAttention # DELETE
...
class MultiSourceAPInputFeedRNNDecoder(DecoderBase):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
        ...
        if not self.attentional:
            ...
            self.ms_attn = MultiSourceAPGlobalAttention(
    ...
    def init_state(self,
        ...
        if isinstance(encoder_final_list[0], tuple):  # LSTM
            self.state["hidden"] = (
                self.hidden_active_0(self.hidden_merge_0(torch.cat([_fix_enc_hidden(encoder_final[0]) for encoder_final in encoder_final_list], dim=2))),
                self.hidden_active_1(self.hidden_merge_1(torch.cat([_fix_enc_hidden(encoder_final[1]) for encoder_final in encoder_final_list], dim=2)))
            )
        else:  # GRU
            self.state["hidden"] = (
                self.hidden_active_0(self.hidden_merge_0(torch.cat([_fix_enc_hidden(encoder_final) for encoder_final in encoder_final_list], dim=2))),
            )
        # end if
