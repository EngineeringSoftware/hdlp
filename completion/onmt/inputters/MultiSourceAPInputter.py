...
import collections
import glob
from itertools import chain, cycle # chain # DELETE
import math
...
class MultiSourceAPInputter:
    ...
    class OrderedIterator(torchtext.data.Iterator):
        ...
        def __init__(self,
            ...
            dataset.sort_key = lambda ex: MultiSourceAPInputter.text_sort_key(ex, src_types)
        ...
        def create_batches(self):
            if self.train:
                if self.yield_raw_example:
                    self.batches = MultiSourceAPInputter.batch_iter(
                        self.data(),
                        1,
                        batch_size_fn=None,
                        batch_size_multiple=1)
                else:
                    self.batches = MultiSourceAPInputter.pool(
                        self.data(),
                        self.batch_size,
                        self.batch_size_fn,
                ...
                self.batches = []
                for b in MultiSourceAPInputter.batch_iter(
                        self.data(),
                        self.batch_size,
    ...
    class MultipleDatasetIterator(object):
        ...
        def __init__(self,
            ...
            for shard in train_shards:
                self.iterables.append(
                    MultiSourceAPInputter.build_dataset_iter(src_types, shard, fields, opt, multi=True))
            self.init_iterators = True
            self.weights = opt.data_weights
            self.batch_size = opt.batch_size
            self.batch_size_fn = MultiSourceAPInputter.max_tok_len \
        ...
        def __iter__(self):
            while True:
                for minibatch in MultiSourceAPInputter.pool(
                        self._iter_examples(),
                        self.batch_size,
                        self.batch_size_fn,
                        self.batch_size_multiple,
    ...
    class DatasetLazyIter(object):
        ...
        def _iter_dataset(self, path):
            MultiSourceAPInputter.logger.info('Loading dataset from %s' % path)
            cur_dataset = torch.load(path)
            MultiSourceAPInputter.logger.info('number of examples: %d' % len(cur_dataset))
            cur_dataset.fields = self.fields
            cur_iter = MultiSourceAPInputter.OrderedIterator(
