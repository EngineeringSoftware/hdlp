...
def model_opts(parser):
    ...
    group.add('--pool_method', '-pool_method', type=str, default='max',
              choices=['max', 'avg', 'fc', 'concat_fc'],
              help="Merge action for incorporating encoder output features. "
                   "Options [max|avg|fc|concat_fc].")
    ...
    # group.add('--multisource', '-multisource', action='store_true',
    #           help="Flag for multisource model to use.")
    group.add('--all_prevassign', '-all_prevassign', action='store_true',
              help="Flag for use of all_prevassign model (MultiSourceAPNMTModel).")
    group.add('--rhsonly', '-rhsonly', action='store_true',
              help="Use of previous assignments with just rhs tokens")
    group.add('--type_append', '-type_append', action='store_true',
              help="Flag for use of MultiSourceTypeAppendedNMTModel")
    group.add('--type_embedding_append', '-type_embedding_append', action='store_true',
              help="Flag for use of with type embedding for MultiSourceTypeAppendedNMTModel")
    group.add('--src_types', '--src_types', nargs='+', default=['l'],
              help="List of type of input source to use for using" 
                   "MultiSourceNMT model."
                   "Options are [l|prevassign]")
...
def preprocess_opts(parser):
    ...
    group.add('--all_prevassign', '-all_prevassign', action='store_true',
              help="Flag for all_prevassign model to use.")
    group.add('--src_types', '-src_types', nargs='+', default=['l'],
              help="List of type of input source to use for using" 
                   "MultiSourceNMT model."
                   "Options are [l|prevassign]")
    ...
    group.add('--use_copy', '-use_copy', action="store_true",
              help='use copy method for MultiSourceDataset class in preprocess.py.')
...
def train_opts(parser):
    ...
    group.add('--consist_reg', '-consist_reg', action='store_true',
              help="Flag for use of consistency regularization")
    group.add('--num_reorder_pa', '-num_reorder_pa', type=int, default=3,
              help="Number of reordered previous assignments given using consistency reg.")
...
def translate_opts(parser):
    ...
    group.add('--avg_raw_probs', '-avg_raw_probs', action='store_true',
              help="If this is set, during ensembling scores from "
                   ...
                   "zero probability.")
    group.add('--use_reranker', '-use_reranker', action='store_true',
              help="Whether to use reranker")
    group.add('--all_prevassign', '-all_prevassign', action='store_true',
              help="Flag for all_prevassign model to use.")
    group.add('--type_append', '-type_append', action='store_true',
              help="Flag for type appended NMT model to use.")

    group = parser.add_argument_group('Data')
    group.add('--data_type', '-data_type', default="text",
              help="Type of the source input. Options: [text|img].")
    group.add('--data_mode', '-data_mode', default="test",
              help="Type of dataset [test|val]")
    group.add('--src', '-src', required=True,
              help="Source sequence to decode (one line per "
                   "sequence)")
    group.add('--src_types', '--src_types', nargs='+', default=['lhs'],
              help="List of type of input source to use for using" 
                   "MultiSourceNMT model."
                   "Options are [l|prevassign]")
    group.add('--src_dir', '-src_dir', default="",
