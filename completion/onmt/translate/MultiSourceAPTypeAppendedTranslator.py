...
from onmt.translate.CustomTranslator import CustomTranslator
from onmt.translate.MultiSourceTranslation import MultiSourceTranslation # DELETE
from onmt.translate.MultiSourceTranslationBuilder import MultiSourceTranslationBuilder
import onmt
...
import time
import torch
from itertools import chain # DELETE

from seutil import LoggingUtils


class MultiSourceAPTypeAppendedTranslator(CustomTranslator):
    ...
    def build_translator(cls, src_types, opt, report_score=True, logger=None, out_file=None):
        ...
        fields, model, model_opt = MultiSourceAPTypeAppendedModelBuilder.load_test_model(src_types, opt)
    ...
    def __init__(self, src_types, model, fields, src_reader, tgt_reader, gpu=-1, n_best=1, min_length=0, max_length=100, ratio=0., beam_size=30, random_sampling_topk=1, random_sampling_temp=1, stepwise_penalty=None, dump_beam=False, block_ngram_repeat=0, ignore_when_blocking=frozenset(), replace_unk=False, phrase_table="", data_type="text", verbose=False, report_bleu=False, report_rouge=False, report_time=False, copy_attn=False, global_scorer=None, out_file=None, report_score=True, logger=None, use_reranker=False, seed=-1):
        ...
        self.logger = MultiSourceAPTypeAppendedModelBuilder.logger
    ...
    def translate(self,
        ...
        data_iter = MultiSourceAPInputter.OrderedIterator(
        ...
        for enc_type, encoder in self.model.encoders.items():
            if enc_type=="l":
                ...
                enc_state, memory_bank, lengths = encoder(src_list[enc_type],
                                                          ...
                                                          src_lengths_list[enc_type])
                enc_states_list.append(enc_state)
                memory_bank_list.append(memory_bank)
                new_src_lengths_list.append(lengths)
        # end for
