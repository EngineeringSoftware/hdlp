...
from onmt.models.MultiSourceAPTypeAppendedModel import MultiSourceAPTypeAppendedModel
...
class MultiSourceAPTypeAppendedModelBuilder:
    ...
    @classmethod
    def build_decoder(cls, opt, num_pas, embeddings):
        dec_type = "ifrnn" if opt.decoder_type == "rnn" and opt.input_feed \
            else opt.decoder_type

        assert dec_type == "ifrnn", "Only input feed rnn is supported"

        opt.src_types.pop(opt.src_types.index(f"type"))
        for i in range(num_pas):
            opt.src_types.pop(opt.src_types.index(f"patype{i}"))
    ...
    def build_base_model(cls,
        ...
        ap_tool_dict = {"sep": fields["src.prevassign0"][0][1].vocab.stoi["<sep>"],
                        "unk": fields["src.prevassign0"][0][1].vocab.stoi["<unk>"],
                        "emp": fields["src.prevassign0"][0][1].vocab.stoi["<empty>"],
                        "pad": fields["src.prevassign0"][0][1].vocab.stoi["<pad>"],
                        "agn": fields["src.prevassign0"][0][1].vocab.stoi["<="]}
        ...
        for src_type in src_types:
            if "type" not in src_type and "patype" not in src_type:
        ...
        # get number of previous assignment in models
        num_pas = 0
        for src_type in src_types:
        ...
        # Build decoder
        decoder = cls.build_decoder(model_opt, num_pas, tgt_emb)
        model_opt.src_types.append(f"type")
        for i in range(num_pas):
            model_opt.src_types.append(f"patype{i}")
        ...
        # end if
        model = MultiSourceAPTypeAppendedModel(encoders,
                                               decoder,
                                               ap_tool_dict,
                                               src_types,
                                               num_pas,
